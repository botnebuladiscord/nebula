...
'''
formated = []        
text = text.split('elif')
for i in text:
    i = i.split(':')
    cmdname_lowercase = i[0].replace('if', '')
    cmdname_lowercase = cmdname_lowercase.replace('elif', '')
    cmdname_lowercase = cmdname_lowercase.replace(' command == ', '')
    cmdname_lowercase = cmdname_lowercase.replace("'", '')
    
    i = i[1].split('\n')
    desc = i[1].split(',')
    desc = desc[1].replace('description=', '')
    desc = desc.replace("'", '')
    if desc == ' color=Color.gold())':
        continue
    desc = desc[1:]
    
    cmd_args = i[2].split(',')
    cmd_args = cmd_args[1].replace('value=', '')
    cmd_args = cmd_args.replace("'", '')
    cmd_args = cmd_args.replace("`", '')
    cmd_args = cmd_args.replace('/'+cmdname_lowercase, '')
    cmd_args = cmd_args[2:]
    
    usage = i[2].split(',')
    usage = usage[1].replace('value=', '')
    usage = usage.replace("'", '')
    usage = usage.replace("`", '')
    usage = usage[1:]
    
    example = i[3].split(',')
    example = example[1].replace('value=', '')
    example = example.replace("'", '')
    example = example.replace("`", '')
    example = example[1:]

    format = f'''<div id="command">
                <div class="command-content" id="{cmdname_lowercase}">
                    <table>
                        <tr>
                            <td id="cmdtext">/{cmdname_lowercase} <code>{cmd_args}</code></td>
                            <td id="icon"><ion-icon name="chevron-down-outline"></ion-icon><ion-icon name="chevron-up-outline" style="display: none;"></ion-icon></td>
                        </tr>
                    </table>
                    <div class="{cmdname_lowercase}" id="content">
                        <p>
                            {desc}<br><b>Usage:</b> {usage}<br><b>Example:</b> {example}
                        </p>
                    </div>
                </div>
            </div>'''
    formated.append(format)
with open('cmds.html', 'w', encoding="utf-8") as file:
    file.writelines(formated)